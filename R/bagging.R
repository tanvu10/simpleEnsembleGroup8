...
#' @param fit_function A function that fits a model and returns a list containing at least coefficients and fitted values.
...
#' data(mtcars)
#' fit_function <- function(y, X) {
#'   model <- lm(y ~ X)
#'   list(coefficients = coef(model), fitted_values = predict(model))
#' }
#' results <- perform_bagging(mtcars$mpg, mtcars[, -1], fit_function, R = 100)
#' print(results)
#'
#' @export
perform_bagging <- function(y, X, fit_function, R) {
  ...
  for (i in 1:R) {
    ...
    coefficients_list[[i]] <- model$coefficients  # Collect coefficients
    predictions_matrix[, i] <- model$fitted_values  # Collect predictions
  ...
  mean_coefs <- colMeans(all_coefs, na.rm = TRUE)
  std_coefs <- apply(all_coefs, 2, sd, na.rm = TRUE)
